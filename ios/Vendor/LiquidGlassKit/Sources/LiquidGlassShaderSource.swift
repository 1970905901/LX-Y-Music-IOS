//
// LiquidGlassShaderSource.swift
// Vendored from DnV1eX/LiquidGlassKit (LiquidGlassVertex.metal / LiquidGlassFragment.metal).
// Embedded as source and compiled at runtime via MTLDevice.makeLibrary(source:) on device:
// the CI Xcode toolchain needs no Metal compile step and no metallib resource bundle.
// (Upstream loads a SwiftPM-precompiled default.metallib, unusable in a CocoaPods static lib.)
//

enum LiquidGlassShaderSource {
...
float xyzToLabNonlinear(float normalizedX) {
    // Threshold: (24/116)^3 鈮?0.00885645167
