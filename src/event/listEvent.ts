...
import { logOperation } from '@/core/sync/opQueue'
...
export class ListEvent extends Event {
  ...
  /**
   * 每日推荐歌曲替换
   * @param oldMusicInfoId 要被替换掉的歌曲ID
   * @param newMusicInfo 替换上的新歌曲信息
   */
  daily_rec_music_replace(oldMusicInfoId: string, newMusicInfo: LX.Music.MusicInfoOnline | null) {
    this.emit('daily_rec_music_replace', oldMusicInfoId, newMusicInfo)
  };
  ...
  async list_data_overwrite(listData: MakeOptional<LX.List.ListDataFull, 'tempList'>, isRemote: boolean = false) {
    ...
    if (!isRemote) {
      await logOperation({ action: 'list_data_overwrite', data: listData })
      markListsChanged()
    }
  };
  ...
  async list_create(position: number, lists: LX.List.UserListInfo[], isRemote: boolean = false) {
    ...
    if (!isRemote) {
      await logOperation({ action: 'list_create', data: { position, listInfos: lists } })
      markListsChanged()
    }
  };
  ...
  async list_remove(ids: string[], isRemote: boolean = false) {
    ...
    checkListExist(changedIds)
    if (!isRemote) {
      await logOperation({ action: 'list_remove', data: ids })
      markListsChanged()
    }
  };
  ...
  async list_update(lists: LX.List.UserListInfo[], isRemote: boolean = false) {
    ...
    if (!isRemote) {
      await logOperation({ action: 'list_update', data: lists })
      markListsChanged()
    }
  };
  ...
  async list_update_position(position: number, ids: string[], isRemote: boolean = false) {
    ...
    if (!isRemote) {
      await logOperation({ action: 'list_update_position', data: { position, ids } })
      markListsChanged()
    }
  };
  ...
  async list_music_overwrite(listId: string, musicInfos: LX.Music.MusicInfo[], isRemote: boolean = false) {
    ...
    if (!isRemote && listId !== LIST_IDS.TEMP) {
      await logOperation({ action: 'list_music_overwrite', data: { listId, musicInfos } })
      markListsChanged()
    }
  };
  ...
  async list_music_add(listId: string, musicInfos: LX.Music.MusicInfo[], addMusicLocationType: LX.AddMusicLocationType, isRemote: boolean = false) {
    ...
    if (!isRemote) {
      await logOperation({ action: 'list_music_add', data: { id: listId, musicInfos, addMusicLocationType } })
      markListsChanged()
    }
  };
  ...
  async list_music_move(fromId: string, toId: string, musicInfos: LX.Music.MusicInfo[], addMusicLocationType: LX.AddMusicLocationType, isRemote: boolean = false) {
    ...
    if (!isRemote) {
      await logOperation({ action: 'list_music_move', data: { fromId, toId, musicInfos, addMusicLocationType } })
      markListsChanged()
    }
  };
  ...
  async list_music_remove(listId: string, ids: string[], isRemote: boolean = false) {
    ...
    if (!isRemote && listId !== LIST_IDS.TEMP) {
      await logOperation({ action: 'list_music_remove', data: { listId, ids } })
      markListsChanged()
    }
  };
  ...
  async list_music_update(musicInfos: LX.List.ListActionMusicUpdate, isRemote: boolean = false) {
    ...
    if (!isRemote) {
      await logOperation({ action: 'list_music_update', data: musicInfos })
      markListsChanged()
    }
  };
  ...
  async list_music_clear(ids: string[], isRemote: boolean = false) {
    ...
    if (!isRemote) {
      await logOperation({ action: 'list_music_clear', data: ids })
      markListsChanged()
    }
  };
